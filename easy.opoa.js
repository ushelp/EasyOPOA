...
(function(window) {
  ...
  replaceFirst = function(hash, url, opoa) {
    if (window.history.pushState) {
      //使用锚记hash的做法
      //也可以直接将浏览器地址栏更改为当前加载的URL，不过这样的话，如果用户点击浏览器刷新按钮就脱离了当前的home.jsp（One Page）
      //所以这里使用#锚记方式实现
  ...
  actionUrlErrors = {}, //动作映射对象（ActionMap Object）,保存全局hash动作映射   "动作hash:[加载的url,EasyOPOA实体]"
  ...
  var EasyOPOA = {
    ...
    addActionMap:function(hash, url, opoa) {
      ...
      if (isArray(hash)) {
        ...
        clearOPOA(opoa2);
        //eg. 
        //EasyOPOA.addActionMap("demo", opoaList.menuOpoa, "demo.jsp" );
        //如果是有参hash
