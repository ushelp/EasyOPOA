 ...
 * Version 2.4.1
 * 
 * http://easyproject.cn https://github.com/ushelp/EasyOPOA
 * 
 ...
 */
        ...
        $.ajax({
            ...
            success:function(data) {
                ...
                if (opoa.find) {
                    ...
                    if (!content) {
                        /*
			 * emptyfind empty 显示空 all 显示加载到的搜有数据 "定义内容" 将自定义内容显示(可使用消息或图片提示用户)
			 * function 执行该函数 如果是函数
			 */
                        if (typeof notFounds[hash] === "function") {
                            ...
                            if (notFounds[hash]) {
                                emptyfind = notFounds[hash].toLowerCase();
                            }
                            // 显示空
                            if (emptyfind == "empty") {
                                content = "";
	 ...
	 * 初始化opoaInstance
	 */
    initOPOA = function(opoaInstance) {
        // 合并问题
        // opoa实体对象
        var opoa = {};
        // 合并opoa实体对象参数
	 ...
	 * 如果开启了最后动作定位的记忆功能，则将动作记录入cookie
	 * 
	 * @param hash
	 *            hash动作
	 * @param url
	 ...
	 * 清理opoa实例中无法保持的函数属性
	 */
    clearOPOA = function(hash, opoa) {
        notFounds[hash] = opoa.notFound;
        actionUrlErrors[hash] = opoa.urlErrors;
        actionLoadings[hash] = opoa.loading;
        delete opoa.notFound;
        delete opoa.loading;
        delete opoa.urlErrors;
    }, /**
	 * hash动作对应没有找到find内容时显示的内容 {"hash动作名称":notFound参数}
	 */
    notFounds = {}, /**
	 * hash动作对应的ajax请求时调用的loading函数 {"hash动作名称":loading处理对象}
	 */
    actionLoadings = {}, /**
	 * hash动作对应的请求发生http错误时如何处理对象 {"hash动作名称":{http状态码:处理函数,……}}
	 */
    actionUrlErrors = {}, /**
	 * 动作映射对象（ActionMap Object）,保存全局hash动作映射 "动作hash:[加载的url,EasyOPOA实体]"
	 */
    actionMaps = {}, /**
	 * 保存含有参hash动作映射 "含参动作hash:[加载的url,EasyOPOA实体]"
	 */
    routerActionMaps = {}, /**
	 * 是否是数组
	 */
	 ...
	 * 带参hash动作参数正则
	 */
    optionalParam = /\((.*?)\)/g, namedParam = /(\(\?)?:\w+/g, splatParam = /\*\w+/g, nameOrsplatParam = /(\(\?)?:\w+|\*\w+/g, escapeRegExp = /[\-{}\[\]+?.,\\\^$|#\s]/g, paramReg = /\{([^}]+)\}/g, spaceReg = /\ /g, intReg = /^[0-9]+$/, /**
	 * 将带参hash转为正则对象
	 * 
	 * @param hash
	 *            hash动作
	 ...
	 */
        Configs:{
            ...
            // 默认值：post
            method:"post",
            // 是否改变浏览器地址栏的hash，用来定位动作，在支持HTML5的浏览器可以实现基于hash的前进后退
            ...
            if (isArray(hash)) {
                $.each(hash, function(k, v) {
                    if (isArray(v)) {
                        ...
                        var v_url = v[1];
                        var v_opoa = v[2];
                        $.extend(true, opoa2, EasyOPOA.Configs, v_opoa);
                        clearOPOA(v_hash, opoa2);
                        // eg.
                        ...
                        // ]);
                        // 如果是有参hash
                        if (v[0].indexOf(":") != -1 || v_hash.indexOf("*") != -1) {
                            routerActionMaps[v_hash] = [ v_url, opoa2 ];
                        } else {
                            actionMaps[v_hash] = [ v_url, opoa2 ];
                        }
                    } else {
                        // eg.
                        // EasyOPOA.addActionMap([
	 ...
	 */
        ...
        /**
	 * 在浏览器第一次请求和解析window.location.hash前执行
	 * 
	 * @param hash
	 *            浏览器第一次加载的hash
	 * @returns {Boolean} 如果返回false，则OPOA引擎不解析第一次地址栏的hash
	 */
        preFirstHash:function(hash) {
            return true;
        },
        /**
	 * 启动EasyOPOA
            ...
            $(function() {
                ...
                if (first) {
                    ...
                    if (window.location.hash) {
                        ...
                        // 如果返回false，则不再解析地址的hash
                        if (!EasyOPOA.preFirstHash(hash)) {
                            return;
                        }
	 ...
	 */
        ...
        /**
	 * 当OPOA没有从Ajax加载首页时，默认显示首页的处理函数 主要用于该场景下浏览器后退到首页时，首页静态内容无法更新问题
	 */
        homeFun:function() {},
        /**
	 * 通过hash动作名称，手动进行加载
	 ...
	 */
        ...
        /**
	 * 手动按顺序加载指定的hash动作名称列表，上一个动作完全加载完成后才加载下一个动作
	 * 
	 * @param loadList
	 *            按顺序加载的动作数组列表，数据格式为：[ [hash, postData], [hash, postData],
	 *            [hash, postData], ...]，postData可选
	 */
        loadLinked:function(loadList) {
            if (isArray(loadList)) {
                var v = loadList[0];
                var hash = v[0];
                var postData = v[1];
                for (var i = 0; i < loadList.length; i++) {
                    // 不是最后一个加载的动作，则加载下一个动作
                    if (i < loadList.length - 1) {
                        var v2 = loadList[i];
                        var hash2 = v2[0];
                        (function(i, hash) {
                            var loading = actionLoadings[hash];
                            var new_loading = {};
                            $.extend(true, new_loading, loading);
                            new_loading.end = function(hash) {
                                loading.end(hash);
                                var v2 = loadList[i + 1];
                                var hash2 = v2[0];
                                var postData2 = v2[1];
                                EasyOPOA.load(hash2, postData2);
                                actionLoadings[hash] = loading;
                            };
                            actionLoadings[hash] = new_loading;
                        })(i, hash2);
                    }
                }
	 ...
	 * 
	 * @param deep
	 *            为空或false仅释放OPOA命名空间，为true 将完全释放EasyOPOA和OPOA命名空间
	 */
